...
## 1. 프로젝트 개요
- 배경: 오늘날 많은 야구팬들은 단순한 관람을 넘어서 더 풍부한 스포츠 경험을 추구하고 있다. 하지만 경기 결과의 불확실성은 관람 만족도를 낮출 수 있으며, 이는 시간과 비용의 낭비로 이어질 수 있다. 본 프로젝트는 이러한 문제를 개선하기 위해 데이터를 기반으로 한 전략적 예매를 제안한다. 특히 엘지트윈스의 승률 패턴을 분석해 관람 시점을 예측 가능하게 만들고자 한다. 분석 대상은 경기 승률에 영향을 주는 요인들과 2025년 전체 승률 예측, 그리고 상대 구단별 홈·원정, 주중·주말에 따른 승률 차이를 포함한다. 팬들에게 보다 효율적이고 만족스러운 관람 기회를 제공하는 것이 핵심 목표다. 
